...
YouTubeReborn_FILES = Tweak.xm $(shell find Controllers -name '*.m') $(shell find AFNetworking -name '*.m') $(shell find Jailbreak-Detection-Lib -name '*.m') $(shell find Alderis -name '*.swift') $(shell find YouTube-Extractor-Kit/YouTubeExtractorKit -name '*.m')
